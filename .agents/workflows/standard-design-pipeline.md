...
### 0. 专家前置审阅 (Review)
- **操作**：调用银发经济专家 `li-yue-expert.md` 审阅迭代计划。
- **具体指令**：请根据 `.agents/instructions/li-yue-expert.md` 审核 `drafts/v{X.Y}/` 的迭代规划，重点关注激励合理性与适老化风险。
...
- **具体指令**：基于 `chatP-ProductArch-expect.md` 定义 `flow-framework.md`，随后基于 `chatA-ProductMgr-expect.md` 编排具体流程并生成 Mermaid 源码。
- **预期结果**：生成 `drafts/v{X.Y}/flow/xxx-flow.html` 及对应的 `.mmd` 文件。
