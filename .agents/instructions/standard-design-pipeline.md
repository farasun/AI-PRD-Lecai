...
- **操作流程**：
  ...
  2. **质量门禁**：唤醒审查员 `Li Yue`（指令：`.agents/instructions/reviewer-liyue.md`），对草拟的总规进行业务合规与适老化体验查问，并结构化输出独立的评审报告文件。**【严格约束：该审查员绝不允许修改 Chat P 编写的 PRD 初稿内容，仅允许输出书面 review】**。
  3. **挂起确认**：生成查问报告后**强制断点停止**，提示等待人类介入处理。
  4. **终稿与装载**：人类完成定调后，再次由 `Chat P` 构建修正后的总纲完稿。**完稿后方可梳理出 `PageList.md`**。此环节存在硬性约束：**页面名称与编号必须绝对以 `foundation/Main_PageList.md` 为准，新增页面需遵循固有编码规范，避免冲突。**最后执行预构建命令：`python .agents/skills/lecai-bundler/scripts/bundle.py drafts/v{X.Y} --init`。
- **预期产出**：
