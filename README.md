...
## 🧩 MockFlow · 低代码体验沙盘（临时实验目录 ⚠️）

> **核心定位**：让人在 30–60 分钟内"假装用低代码工具搭一个应用"，从而直观理解低代码是什么。
> **特别说明**：这是一个临时存放在项目中的 React Web 实验应用，主要用于方便 AI 助手（如 Claude Code）进行低代码交互逻辑的探索与开发验证，**不属于 PRD 资产库的正式核心组成部分**。请勿与 PRD 主线产生强逻辑绑定。

MockFlow 是本项目的**交互式低代码教学原型**，仿照轻流（Qingflow）的 UI 交互模式，内置一套数据驱动的**引导脚本引擎**，让用户手把手走完"建表单 → 配流程 → 发布使用 → 看数据"的完整低代码搭建动线。

### 特性亮点

| 模块 | 能力 |
|------|------|
| 🎓 引导脚本引擎 | 信息卡 / 高亮聚焦 / 强制操作 / 分支选择 / 成果展示 五种步骤类型 |
| 📝 表单设计器 | 真实拖拽字段到画布，字段属性右侧配置，可排序删除 |
| 🔄 流程设计器 | 可视化节点链，点击 + 插入审批/抄送/条件分支节点 |
| 🗄️ 数据管理 | 表格视图，预置种子数据 + 用户提交记录，支持筛选搜索 |
| 📊 报表 & 仪表盘 | Recharts 驱动的真实图表，支持拖拽报表卡片到仪表盘 |
| 🚀 使用端闭环 | 员工提交申请 → 主管切身审批 → 数据沉淀全链路体验 |
| 🤖 帮我做 | 任意步骤一键自动完成，绝不卡死，永远有兜底 |
...
React 18 + TypeScript + Tailwind CSS + dnd-kit + Recharts + Zustand
...
├── dist/                   ← 打包分发产物 (.html)
├── mockflow/               ← 临时实验目录 (低代码沙箱，AI 开发实验用)
└── scripts/                ← 全局构建与维护工具
```
